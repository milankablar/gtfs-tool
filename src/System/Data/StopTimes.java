...
public class StopTimes {
    ...
    public HashMap<String, LinkedList<StopTime>> stopTimesMap;
    public ArrayList stopTimesList;
    public ArrayList<String> headerList;
    public final ArrayList<String> sampleHeaderList = new ArrayList<>(Arrays.asList("trip_id",
    ...
    /**
     * Accessor method for stopTimes HashMap
     * @return stopTimesMap
     */
    public HashMap<String, LinkedList<StopTime>> getStopTimesMap() {
        return stopTimesMap;
    }
    ...
    public String importFile(Path path) throws IOException {
        ...
        while (stopTimesScanner.hasNextLine()) {
            ...
            if(stopTimeList.size() == 0) {
                ...
                if(!stopTimesMap.containsKey(stopTimeList.get(0))){
                    stopTimesMap.put(stopTimeList.get(0), new LinkedList<StopTime>());
                }
                stopTimesMap.get(stopTimeList.get(0)).add(stopTime);
            }
        }
        return "error-free";
    }
	...
	public List<StopTime> searchByStop(String stopId){
        ...
        while(iterator.hasNext()){
            ...
            List<StopTime> temp = hasID(currentEntry.getValue(), stopId, "stopId");
            if(temp.size() != 0){
	...
	public List<StopTime> searchByTrip(String tripId){
        ...
        while(iterator.hasNext()){
            Map.Entry<String, LinkedList<StopTime>> currentEntry = iterator.next();
            List<StopTime> temp = hasID(currentEntry.getValue(), tripId, "tripId");
            if(temp.size() != 0){
                stopTimeList.addAll(temp);
            }
        }
        return stopTimeList;
	}
     ...
     * @author Jack Haek
     */
    private List<StopTime> hasID(List<StopTime> list, String id, String type){
        List<StopTime> rtnList = new ArrayList<>();
